...
async function get_favorites_ids(category_name,user_id){
  const favorites_ids = await DButils.execQuery(
    `SELECT ${category_name}_id FROM dbo.favorite_${plural(category_name)} WHERE user_id=${user_id}`
  ).then((favorites_ids) =>
...
async function verify_category(category_name,categories){
  const category_match = Object.values(categories).find(possible_category => possible_category === category_name);
