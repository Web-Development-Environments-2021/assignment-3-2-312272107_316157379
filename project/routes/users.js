...
router.use(async function (req, res, next) {
  try {
    if (req.session && req.session.user_id) {
      await DButils.execQuery("SELECT user_id FROM dbo.users").then((users) => {
...
router.post("/favorites/:category_name", async (req, res, next) => {
  try {
    ...
    await users_utils.verify_category(
      category_name,
      users_utils.favorite_categories
    );
...
router.get("/favorites/:category_name", async (req, res, next) => {
  try {
    ...
    const favorites_ids = await users_utils.get_favorites_ids(
      category_name,
      user_id
    );

    // get and use the correct function to get the favorites using the ids extracted based on the category.
    const favorites_utils = await users_utils.get_utils_by_category(
      category_name
    );

    const favorites_info = await favorites_utils.get_info(
      favorites_ids,
      "favorites"
    ); // get info to display based on category
